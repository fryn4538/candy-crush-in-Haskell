...
window :: Display
window = InWindow "Candy Crush" (width, height) (offset, offset)
...
data CandyGame = Game {squareLoc :: (Float, Float) 
                      ,candy :: [((Float, Float),Int)]} deriving Show

--  Draw a pong game state (convert it to a picture).
render :: CandyGame ->  Picture
render game = pictures ((paintRectangles (squareLocations 5 (-200,200))) ++ [mkMarker rose $ squareLoc game] ++ (paintCandy (candyLocations 5 (-200,200)))) -- Konkatinerade rutnätet med Eriks ruta

  
mkMarker :: Color -> (Float, Float) -> Picture
mkMarker col (x,y) = pictures
  [ translate x y $ color markerColor $ rectangleSolid 99 99
  ]

markerColor = dark (dark red)

handleCandy :: Int -> CandyGame
handleCandy a | a <= 0 = []
              | a > 0 = (candyLocations 0 ((-200),200)), 1)

{-
mkCandy :: Int -> Picture
mkCandy boardSize =  pictures
  [ translate 0 0 $ color black $ rectangleSolid 100 100]
...
paintCandy [] = []
paintCandy ((a,b):xs) = [Color red $ translate a b $ rectangleSolid 50 50] ++
                             paintCandy xs
{-
...
candyLocations 0  (a,b)
  | a > 200 && b < (-200) = []
  | otherwise = candyLocations 5 ((-200), b-100)
...
initialState = Game { squareLoc = ((-200),200)
                     }
